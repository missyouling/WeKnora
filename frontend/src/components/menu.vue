                ...
                <t-tooltip :content="item.title" placement="right" :disabled="!uiStore.sidebarCollapsed">
                    <div @click="handleMenuClick(item.path)" @mouseenter="mouseenteMenu(item.path)"
                        ...
                        <div class="menu_item-box">
                            <div class="menu_icon">
                                <img class="icon"
                                    :src="getImgSrc(item.icon == 'zhishiku' ? knowledgeIcon : item.icon == 'agent' ? agentIcon : item.icon == 'organization' ? organizationIcon : item.icon == 'logout' ? logoutIcon : item.icon == 'setting' ? settingIcon : item.icon == 'daily-affairs' ? dailyAffairsIcon : prefixIcon)"
                                    alt="">
                            </div>
...
const isMenuItemActive = (itemPath: string): boolean => {
    ...
    switch (itemPath) {
        ...
        case 'daily-affairs':
            return currentRoute === 'dailyAffairs' || currentRoute === 'invoiceManagement';
        case 'creatChat':
...
const topMenuItems = computed<MenuItem[]>(() => {
    return (visibleMenuArr.value as unknown as MenuItem[]).filter((item: MenuItem) =>
        item.path === 'knowledge-bases' || item.path === 'agents' || item.path === 'organizations' || item.path === 'creatChat' || item.path === 'daily-affairs'
    );
});

const bottomMenuItems = computed<MenuItem[]>(() => {
    return (visibleMenuArr.value as unknown as MenuItem[]).filter((item: MenuItem) => {
        if (item.path === 'knowledge-bases' || item.path === 'agents' || item.path === 'organizations' || item.path === 'creatChat' || item.path === 'daily-affairs') {
            return false;
        }
        return true;
    });
...
let dailyAffairsIcon = ref('daily-affairs.svg');
...
const getIcon = (path: string) => {
    ...
    const dailyAffairsActiveState = route.name === 'dailyAffairs' || route.name === 'invoiceManagement';
    ...
    // 日常事务图标：只在日常事务相关页面显示绿色
    dailyAffairsIcon.value = dailyAffairsActiveState ? 'daily-affairs-green.svg' : 'daily-affairs.svg';
