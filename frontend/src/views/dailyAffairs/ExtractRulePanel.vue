...
const DEFAULT_RULES: Record<string, Record<string, Record<string, RuleDraft>>> = {
  vehicle: {
    ...
    道路运输证: {
      ...
      审验状态: { desc: '审验是否通过', rule: '识别“已审验 / 未审验 / 合格”等表述；原文无则留空' },
    ...
    保险单: {
      ...
      到期提醒天数: { desc: '提前提醒天数', rule: '原文无则留空，不猜测' },
      是否续保: { desc: '是否已续保', rule: '识别“是 / 否”；原文无则留空' },
      备注: { desc: '补充说明', rule: '原文中其它重要信息摘要，不超过 50 字；无则留空' },
    },
  },
  driver: {
    驾驶证: {
      ...
      到期提醒天数: { desc: '提前提醒天数', rule: '原文无则留空，不猜测' },
      提醒状态: { desc: '未提醒、已提醒、已办理等', rule: '原文无则留空，不猜测' },
      备注: { desc: '补充说明', rule: '原文中其它重要信息摘要，不超过 50 字；无则留空' },
    },
    从业资格证: {
      ...
      到期提醒天数: { desc: '提前提醒天数', rule: '原文无则留空，不猜测' },
      提醒状态: { desc: '未提醒、已提醒、已办理等', rule: '原文无则留空，不猜测' },
      审验状态: { desc: '审验是否通过', rule: '识别“已审验 / 未审验”等表述；原文无则留空' },
      备注: { desc: '补充说明', rule: '原文中其它重要信息摘要，不超过 50 字；无则留空' },
    },
  },
  maintain: {
