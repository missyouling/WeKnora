...
type pgRepository struct {
	...
	// bm25 availability is probed once (pg_indexes lookup for the ParadeDB
	// embeddings_search_idx). Hosted Postgres such as Supabase does not ship
	// pg_search, in which case keyword retrieval falls back to an ILIKE scan.
	bm25Once  sync.Once
	hasBM25   bool
	bm25Probe error
}
...
// detectBM25 reports whether the ParadeDB bm25 index (pg_search) is available.
func (g *pgRepository) detectBM25(ctx context.Context) bool {
	g.bm25Once.Do(func() {
		var cnt int64
		if err := g.db.WithContext(ctx).
			Raw("SELECT count(*) FROM pg_indexes WHERE indexname = 'embeddings_search_idx'").
			Scan(&cnt).Error; err != nil {
			g.bm25Probe = err
			logger.GetLogger(ctx).Warnf("[Postgres] detect bm25 index failed: %v", err)
			return
		}
		g.hasBM25 = cnt > 0
		if !g.hasBM25 {
			logger.GetLogger(ctx).Warnf("[Postgres] bm25 index (pg_search) not found; keyword search falls back to ILIKE scan")
		}
	})
	return g.hasBM25
}
...
func (g *pgRepository) KeywordsRetrieve(ctx context.Context,
	...
	// Keyword matching: use ParadeDB's ||| operator when the bm25 index
	// (pg_search) is present; otherwise fall back to a plain ILIKE scan so
	// hosted Postgres (e.g. Supabase) without pg_search still works.
	useBM25 := g.detectBM25(ctx)
	var scoreSQL string
	if useBM25 {
