...
// SaveInvoiceCustomMetadata persists an invoice extraction result into a
// knowledge entry's custom_metadata WITHOUT the generic flat-scalar validation
// applied by UpdateKnowledge (which rejects nested arrays/objects such as the
// invoice "invoices" list). This is a dedicated write path for the invoice
// management module; it does not enqueue a summary refresh.
...
// invoice category tags to a knowledge entry, preserving its existing manual
// tags. Non-fatal by design: an extraction failure must not block saving.
func (s *knowledgeService) EnsureInvoiceCategoryTags(ctx context.Context, kbID, knowledgeID string, categories []string) error {
	...
	// Preserve existing (manual) tags, then append category tags.
	existing := make(map[string]struct{})
	tagIDs := make([]string, 0, len(names)+4)
	tagMap, err := s.repo.GetKnowledgeTags(ctx, []string{knowledgeID})
	if err != nil {
		return fmt.Errorf("get knowledge tags: %w", err)
	}
	for _, t := range tagMap[knowledgeID] {
		if t == nil {
			continue
		}
	...
	for _, name := range names {
		...
		if err != nil {
			return fmt.Errorf("find or create tag %q: %w", name, err)
		}
		if tag == nil {
			continue
		}
