...
2. 一份奖惩通报可能涉及多个当事人（如一份通报处罚多人，正文通常以"（一）（二）（三）"小节分别叙述每个人的违规事实与处理决定）。必须为每个当事人各输出一条记录到 records 数组，禁止把多个人的信息合并进同一条记录；正文没有当事人维度时（纯通报/通知）输出一条记录。
...
6. ap_type 只从以下枚举中选择：处罚、奖励、通报、其它奖惩。根据当事人对应的处理内容判断：含「处罚」「罚款」「警告」「处分」「批评」「扣款」「记过」→处罚；含「奖励」「表彰」「嘉奖」「表扬」「奖金」「评优」「晋级」→奖励；文档为纯通报/通知性质（无具体奖惩决定）→通报；都不明确时用「其它奖惩」。
7. person 为当事人姓名（如"汪艳"），一项处理对应一个人；dept 为当事人所属部门；position 为岗位；无法识别时返回空字符串。每个当事人在 records 中单独一条：该条记录只填这个人自己的 person/dept/position/amount/measure/basis。
8. amount 为金额（如"200元""罚款200元""奖励500元"，保留原文金额数字和单位，无金额返回空字符串）；measure 为措施（如"经济罚款""书面警告""嘉奖""取消评优资格"，简要概括）；basis 为依据（如"依据《AD-018奖惩制度》"，可含制度名）；无法识别时返回空字符串。多个当事人各自的金额/措施填在各自那条记录里，不要互相混入。
...
输出格式：
{"kind":"award_punish","records":[{"ap_no":"","ap_title":"","ap_type":"","person":"","dept":"","position":"","amount":"","measure":"","basis":"","signer":"","sign_date":"","effective_date":"","remark":"","summary":""}]}

示例（一份通报处罚两人，输出两条记录，各填各的 person/amount/measure）：
{"kind":"award_punish","records":[
{"ap_no":"星达行政字〔2026〕11号","ap_title":"关于对汪艳、王郑红违规行为的处罚通报","ap_type":"处罚","person":"汪艳","dept":"仓库","position":"仓库主管","amount":"200元","measure":"经济罚款, 书面警告","basis":"依据《AD-018奖惩制度》","signer":"廖宏","sign_date":"2026-07-01","effective_date":"","remark":"","summary":"汪艳因重复操作失误被处以经济罚款200元并给予书面警告。"},
{"ap_no":"星达行政字〔2026〕11号","ap_title":"关于对汪艳、王郑红违规行为的处罚通报","ap_type":"处罚","person":"王郑红","dept":"质管部","position":"","amount":"300元","measure":"经济罚款, 书面警告","basis":"依据《AD-018奖惩制度》及《质管部绩效考核明细表》","signer":"廖宏","sign_date":"2026-07-01","effective_date":"","remark":"取消年度评优资格","summary":"王郑红因夜班玩手机、睡觉被处以经济罚款300元并给予书面警告。"}]}`
