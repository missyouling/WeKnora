...
7. 费用明细 fee_items：账单"电费明细"表格（费用类别/费用组成/分时时段/计费电量/计费标准/电费）中每一行"费用组成"都必须提取为一条，逐行列出，不得合并、不得省略、不得只取部分、不得截断。注意：文本中"费用组成"行之间可能夹着用能分析等无关文字，也要把表格行按原顺序全部找出来；类别小计行（如"(4)系统运行费 31633.19"后面跟着"其中："子行的）不要单独输出，改输出其下每个子行（如"抽水蓄能容量电费 3666.82"）；0 金额行（如"辅助服务费用 0"）也要输出；返还/损益类行（如"零售交易发电侧收益返还电费""中长期超额申报费用返还（用户）""中长期偏差回收返还（用户）""零售损益分摊电费""上网环节线损代理采购损益"）即使金额为负也必须逐条输出，不得遗漏。该账单费用明细行数通常在 30~50 行，如果输出少于 15 条，说明有遗漏，请补全。category 为该行所属费用类别：尽量使用"(1)市场化购电费""(2)上网环节线损费用""(3)输配电量电费""(4)系统运行费""(5)目录电费""(6)政府性基金及附加""功率因数调整电费"；政府性基金及附加必须区分工商业与居民：若该行 qty 对应的计费电量与工商业分时电量（尖峰+峰+平+谷之和，本账单约 54.7 万千瓦时）一致则为"(6)政府性基金及附加-工商业"，若与居民目录电量（本账单 8330 千瓦时）一致则为"(6)政府性基金及附加-居民"；name 为该行费用组成名称；period 为分时时段（尖峰/峰/平/谷，无时段填""）；qty 为该行计费电量；rate 为该行计费标准/单价；fee 为该行电费金额（负数保留负号，返还/偏差/调整类金额照抄原值）。fee_items 按账单出现顺序输出。
...
const utilityBillExtractionOutputSchema = `{"type":"object","properties":{"kind":{"type":"string"},"records":{"type":"array","items":{"type":"object","properties":{"bill_period_start":{"type":"string"},"bill_period_end":{"type":"string"},"account_no":{"type":"string"},"account_name":{"type":"string"},"address":{"type":"string"},"usage_category":{"type":"string"},"voltage_level":{"type":"string"},"supply_unit":{"type":"string"},"market_attr":{"type":"string"},"print_date":{"type":"string"},"total_kwh":{"type":"number"},"total_amount":{"type":"number"},"prev_kwh":{"type":"number"},"mom_change":{"type":"string"},"avg_price":{"type":"number"},"power_factor":{"type":"number"},"due_date":{"type":"string"},"industrial_amount":{"type":"number"},"residential_amount":{"type":"number"},"pf_adjust_amount":{"type":"number"},"grand_total":{"type":"number"},"deep_peak_kwh":{"type":"number"},"peak_kwh":{"type":"number"},"flat_kwh":{"type":"number"},"valley_kwh":{"type":"number"},"reactive_kwh":{"type":"number"},"capacity":{"type":"number"},"capacity_price":{"type":"number"},"capacity_fee":{"type":"number"},"demand":{"type":"number"},"pf_standard":{"type":"number"},"adjust_ratio":{"type":"number"},"fee_items":{"type":"array","items":{"type":"object","properties":{"category":{"type":"string"},"name":{"type":"string"},"period":{"type":"string"},"qty":{"type":"number"},"rate":{"type":"number"},"fee":{"type":"number"}},"required":["category","name","period","qty","rate","fee"]}},"remark":{"type":"string"}},"required":["bill_period_start","bill_period_end","account_no","account_name","address","usage_category","voltage_level","supply_unit","market_attr","print_date","total_kwh","total_amount","prev_kwh","mom_change","avg_price","power_factor","due_date","industrial_amount","residential_amount","pf_adjust_amount","grand_total","deep_peak_kwh","peak_kwh","flat_kwh","valley_kwh","reactive_kwh","capacity","capacity_price","capacity_fee","demand","pf_standard","adjust_ratio","fee_items","remark"]}},"required":["kind","records"]}}`
