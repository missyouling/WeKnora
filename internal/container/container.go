...
func initDatabase(cfg *config.Config) (*gorm.DB, error) {
	...
	case "postgres":
		// SSL mode is configurable via DB_SSLMODE (default "disable" keeps the
		// historical behaviour). Hosted Postgres such as Supabase requires
		// sslmode=require for direct connections, so operators can set
		// DB_SSLMODE=require without editing code.
