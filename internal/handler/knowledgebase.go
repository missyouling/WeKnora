...
// ListInvoiceRecords godoc
// @Summary      发票级聚合列表
// @Description  返回知识库下所有已提取发票的聚合列表（一行一张发票），服务端完成按发票号去重、全字段搜索、类型/状态/开票日期筛选、排序、分页与金额聚合。
// @Tags         知识库
// @Accept       json
// @Produce      json
// @Param        id           path  string  true  "知识库ID"
// @Param        q            query string  false "全字段搜索词"
// @Param        invoice_type query string  false "发票类型（5 枚举）"
// @Param        status       query string  false "提取状态"
// @Param        date_from    query string  false "开票日期起 YYYY-MM-DD"
// @Param        date_to      query string  false "开票日期止 YYYY-MM-DD"
// @Param        page         query int     false "页码"
// @Param        page_size    query int     false "每页条数"
// @Success      200  {object}  map[string]interface{}  "聚合列表"
// @Security     Bearer
// @Security     ApiKeyAuth
// @Router       /knowledge-bases/{id}/invoices [get]
func (h *KnowledgeBaseHandler) ListInvoiceRecords(c *gin.Context) {
	ctx := c.Request.Context()
	...
	result, err := h.knowledgeService.ListInvoiceRecords(effCtx, kbID, filter)
	if err != nil {
...
// ListUtilityBillRecords godoc
// @Summary      电费账单记录列表
// @Description  聚合返回该知识库下所有电费账单记录（每份账单一条），支持关键字/日期/状态筛选、分页。
// @Tags         知识库
// @Accept       json
// @Produce      json
// @Param        id          path   string                true  "知识库ID"
...
// @Success      200  {object}  types.UtilityBillListResult "记录列表"
// @Failure      403  {object}  errors.AppError            "无权限"
// @Security     Bearer
// @Security     ApiKeyAuth
// @Router       /knowledge-bases/{id}/utility-bill-records [get]
func (h *KnowledgeBaseHandler) ListUtilityBillRecords(c *gin.Context) {
	ctx := c.Request.Context()
	...
	result, err := h.knowledgeService.ListUtilityBillRecords(effCtx, kbID, filter)
	if err != nil {
...
// ListInvoiceTaxRates godoc
// @Summary      发票税率列表
// @Description  返回知识库下所有发票出现过的去重税率（含多档明细），用于税率筛选下拉自动加载。
// @Tags         知识库
// @Accept       json
// @Produce      json
// @Param        id path string true "知识库ID"
// @Success      200 {object} map[string]interface{} "税率列表"
// @Security     Bearer
// @Security     ApiKeyAuth
// @Router       /knowledge-bases/{id}/invoice-tax-rates [get]
func (h *KnowledgeBaseHandler) ListInvoiceTaxRates(c *gin.Context) {
	ctx := c.Request.Context()
	_, kbID, effectiveTenantID, _, err := h.validateAndGetKnowledgeBase(c)
	if err != nil {
		c.Error(err)
	...
	effCtx := context.WithValue(ctx, types.TenantIDContextKey, effectiveTenantID)
	rates, err := h.knowledgeService.ListInvoiceTaxRates(effCtx, kbID)
	if err != nil {
