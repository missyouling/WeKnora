...
func RegisterKnowledgeRoutes(r *gin.RouterGroup, handler *handler.KnowledgeHandler, g *rbacGuards) {
	...
	kbRead := kb.With(apiKeyRetrieve(apiKeyFullAccess()))
	{
		...
		// 发票字段提取 — 读取已解析文本调用提取模型（复用 KB 的 summary_model_id），
		// 结果写入 custom_metadata。属于对 KB 内容的写操作，与上传同权限矩阵。
		kb.POST("/:knowledgeId/extract-invoice", g.OwnedKBOrAdmin(), g.KBAccessWrite("id"), handler.ExtractInvoice)
		// 按页重新提取发票 — 与 extract-invoice 同权限矩阵（写操作）
