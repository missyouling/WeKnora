...
BEGIN
    ...
    -- pg_search (ParadeDB BM25) is OPTIONAL: hosted Postgres such as Supabase
    -- does not ship it. When unavailable, the bm25 index below is skipped and
    -- keyword retrieval in the postgres retriever falls back to an ILIKE scan.
    IF EXISTS (SELECT 1 FROM pg_available_extensions WHERE name = 'pg_search') THEN
        CREATE EXTENSION IF NOT EXISTS pg_search;
        RAISE NOTICE '[Migration 000002] pg_search available; enabling BM25 keyword index';
    ELSE
        RAISE NOTICE '[Migration 000002] pg_search NOT available; BM25 keyword index will be skipped';
    END IF;
    ...
    -- Create BM25 search index (check if exists first and pg_search is available)
    IF EXISTS (SELECT 1 FROM pg_available_extensions WHERE name = 'pg_search')
       AND NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'embeddings_search_idx') THEN
